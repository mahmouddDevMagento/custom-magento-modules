...
namespace BraveBison\Gtag\Plugin\Quote;
...
class SetGtagOnCartRemove
{
    ...
    public function afterRemoveItem(Quote $subject, $result, $itemId)
    {
        ...
        if ($item) {
            $this->setItemForTriggerRemoveEvent($this->registry, $item, $item->getQty());
        }
    ...
    )
    {
        ...
        if ($qty > $result->getQty() && (int)$itemId === (int)$result->getItemId()) {
            $this->setItemForTriggerRemoveEvent($this->registry, $result, $qty - $result->getQty());
        }
    ...
    {
        $namespace = 'bb_products_to_remove';
